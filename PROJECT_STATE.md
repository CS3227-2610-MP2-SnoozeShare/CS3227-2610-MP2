...
| C29 | 2026-09-26 | Audit Log screen filters are ONE search box + action-type combo + From/To date range (replaces the board's User ID and Booking ID inputs). Name text is resolved to user ids first and the log queried by id, so history survives renames (names are also snapshotted on each row). System-initiated rows are attributed to a seeded non-loginable System user (actorUserId stays NOT NULL). | Operator | Operator conversation, 2026-09-26; W12 spec § 5–6 |
